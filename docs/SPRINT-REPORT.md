...
1. **Directory structure** — `demo/`、`assets/css`、`assets/js`、`assets/vendor`、`docs/`、`tests/`。主8画面＋個人設定＋index/詳細/2ページ目/送信契約を含む21 HTML。
...
8. **Design tokens** — 56px header / 232px sidebar / 24px content / body16px / title26px / row52px / control38px / radius4–6px。spacingは4/8/12/16/20/24/32/48px。
9. **Gray Theme** — theme #343b45、active #e0e3e7、sidebar #f0f1f3。Soft blue、Muted red、Darkの比較プレビューも追加。他のbrand token overrideも定義。
10. **Action / semantic colors** — 標準primary #315fbd、Muted redのprimary #86515a。success / warning / danger / infoの意味はテーマ間で共通。Darkでは暗いsurfaceに合わせて明るい状態文字色と濃いdangerボタン色を使用。
...
25. **Offline validation** — 全参照ローカル、外部通信不可CSP下で動作。21ページ600参照の静的チェックPASS。詳細条件/未実施範囲はVALIDATION.md。
26. **Accessibility** — landmarks/skip/focus/labels/ARIA/table semantics/reduced motion。modalのfocus trap/復帰を操作確認。Gray/Soft blue/Muted red/Darkを含む主要24色組は4.5:1以上。
