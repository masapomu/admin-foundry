...
Soft blueとMuted redは比較用accent presetで、同じGraphite shellを共有する。Muted redでもdangerの意味色は別。Darkは既存の比較用presetとして`data-bs-theme="dark"`も併用する。Reference SiteのURLパラメーターはdemo専用で、実アプリではhostが初期HTMLに属性を描画する。
...
## 20. Buttons
Primaryは標準で明確な青、secondaryは白いsurface、link/ghostは低い視覚重量、dangerは意味色の赤。Soft blueとMuted redは比較用accentを変える。Darkのdangerボタンは明るい状態文字色と分け、白文字が読める濃い赤を使う。標準38px、radius6px。アイコンだけの操作にはaria-label。disabled理由は近くの文章で説明する。
