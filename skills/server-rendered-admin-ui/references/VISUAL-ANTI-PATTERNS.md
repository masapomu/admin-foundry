...
16pxの本文、26pxのページ見出し、52px基準の表、6–10pxの用途別radius、有限spacing scale。Graphite BlueではGraphite shellと明るいcontentを分け、primary actionとselected stateへblueを用いる。Semantic/Chart paletteはaccentから独立させ、薄い罫線だけに構造を任せない。Sapphire Blue、Garnet Red、Aqua Ivory、Darkの比較でも同じ情報密度を維持する。各テーマの淡いgradientはsidebarのsurface表現に限定する。カードは意味のあるまとまりだけに使う。
