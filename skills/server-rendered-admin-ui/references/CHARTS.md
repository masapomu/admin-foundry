...
配置は`assets/vendor/chartjs/chart.umd.min.js`、source map、`LICENSE.md`、同梱@kurkle/color 0.3.2の`COLOR-LICENSE.md`。更新元とライセンスはリポジトリ直下の`THIRD-PARTY-NOTICES.md`参照。
...
## 7. Colors
light系列色はblue #2866a5、teal #287a78、violet #74619b、orange #a4632c、rose #a04f68。Darkは対応する明るい色へ切り替える。danger redを無意味な系列に使わない。Graphite Blue/Sapphire Blue/Garnet Redでも系列paletteは共通で、application accentとsemantic colorsから独立する。比較用presetではgridとtooltip面だけを各content/shell toneに合わせる。テーマ属性は初期描画前にhostが出力する。動的変更時は`AdminCharts.refresh(canvas)`でtokenを読み直す。

Aqua Ivoryでは淡い水色の参考に合わせ、series 1のみ#1b78ad、grid #dce6ea、tooltip #243946を使う。他の系列とsemantic colorsは共通のまま。
...
<script defer src="../vendor/chartjs/chart.umd.min.js"></script>
